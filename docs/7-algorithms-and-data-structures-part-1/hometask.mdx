...
<TableForMentors data={[
  {
    ...
    items: [
      {
        text: "Graph generator is implemented correctly",
        points: 1,
      },
      {
        text: "`findShortestPath` method is implemented correctly",
        points: 1,
      },
      {
        text: "`findAllShortestPaths` method is implemented correctly",
        points: 1,
      },
      {
        text: "`findShortestPath` and `findAllShortestPaths` work for edge cases (for case when no path can be found, for small and big datasets)",
        points: 2,
      },
    ]
  },
